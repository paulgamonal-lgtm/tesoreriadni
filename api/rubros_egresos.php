...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $stmt = $conn->prepare("SELECT * FROM rubros_egresos WHERE id = ?");
            ...
            $sql = "SELECT * FROM rubros_egresos";
            if ($periodo_id) {
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO rubros_egresos (nombre, descripcion, periodo_id) VALUES (?, ?, ?)");
    ...
    case 'PUT':
        ...
        $stmt = $conn->prepare("UPDATE rubros_egresos SET nombre = ?, descripcion = ?, activo = ? WHERE id = ?");
    ...
    case 'DELETE':
        $id = intval($_GET['id']);
        $stmt = $conn->prepare("DELETE FROM rubros_egresos WHERE id = ?");
        $stmt->bind_param("i", $id);
